...
namespace CrypticPay.ProfilePhoto
{
    public class BlobStorageService
    {
        ...
        public BlobStorageService(IOptions<StorageAccountOptions> settings, UserManager<CbgUser> userManager)
        {
            _settings = settings.Value;
            _userManager = userManager;
            accessKey = _settings.StorageAccountConnectionString;
        }
        ...
        private async Task<string> UploadFileToBlobAsync(string strFileName, byte[] fileData, string fileMimeType)
        {
            try
            {
                CloudStorageAccount cloudStorageAccount = CloudStorageAccount.Parse(accessKey);
                CloudBlobClient cloudBlobClient = cloudStorageAccount.CreateCloudBlobClient();
                string strContainerName = _settings.FullImagesContainerNameOption;
                CloudBlobContainer cloudBlobContainer = cloudBlobClient.GetContainerReference(strContainerName);
